    ...
    <section className="section bg-white py-24">
      <div className="container mx-auto px-4">
        <div className="text-center mb-16 reveal">
          <span className="inline-block px-3 py-1 text-xs font-medium bg-flex-green/10 text-flex-green-dark rounded-full">
            Core Features
          </span>
          <h2 className="section-title mt-4 text-flex-text">
            Transform Your Habits <span className="text-flex-green">Seamlessly</span>
          </h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {features.map((feature, index) => (
            ...
            >
              ...
              <h3 className="text-xl font-bold mb-2 text-flex-text">{feature.title}</h3>
              <p className="text-foreground/70">{feature.description}</p>
            </div>
          ))}
        </div>

        <div className="mt-20 p-8 rounded-2xl bg-flex-light reveal">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 items-center">
            <div>
              <h3 className="text-2xl font-bold mb-4 text-flex-text">Adaptive Workflow</h3>
              ...
              <ul className="space-y-3">
                {[
                  ...
                  <li key={index} className="flex items-center">
                    <CheckCircle className="w-5 h-5 text-flex-green mr-2 flex-shrink-0" />
                    <span>{item}</span>
                  </li>
                ))}
              </ul>
            ...
            <div className="flex justify-center">
              <div className="relative w-full max-w-md h-[300px]">
                <div className="absolute top-0 left-0 right-0 h-[200px] bg-white rounded-xl shadow-md p-4">
                  <div className="w-full h-4 bg-flex-green/20 rounded-full mb-4">
                    <div className="h-full w-3/4 bg-flex-green rounded-full"></div>
                  </div>
                ...
                <div className="absolute top-[100px] left-[50px] right-0 h-[200px] bg-white rounded-xl shadow-md p-4 transform -rotate-3 z-10">
                  <div className="flex items-center mb-4">
                    <div className="w-8 h-8 rounded-full bg-flex-yellow/30 mr-3"></div>
                  ...
                  <div className="flex justify-between mb-3">
                    <LineChart className="w-5 h-5 text-flex-green" />
                    <div className="text-xs font-medium text-flex-green">+15%</div>
                  </div>
                  <div className="w-full h-20">
                    <div className="flex items-end justify-between h-full px-2">
                      {[35, 50, 70, 45, 60, 80, 65].map((height, i) => (
                        <div 
                          key={i} 
                          className="w-[8%] bg-flex-gradient rounded-sm"
                          style={{ height: `${height}%` }}
                        ></div>
                      ))}
                    </div>
                ...
                {/* Added Flex branding illustration */}
